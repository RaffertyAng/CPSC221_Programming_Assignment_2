...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
	...
	int W = config.img.width(), H = config.img.height();
	vector<vector<bool>> vis(W, vector<bool>(H, 0));
	RGBAPixel base = config.seedpoint.color;
	os.Add(config.seedpoint);
	while (!os.IsEmpty()) {
		PixelPoint p = os.Peek();
		os.Remove();
		int x = p.x, y = p.y;
		vis[x][y] = 1;
		framecount++;
		RGBAPixel* px = config.img.getPixel(x, y);
		if (y - 1 >= 0 && base.distanceTo(*(config.img.getPixel(x, y - 1))) <= config.tolerance && !vis[x][y - 1]) {
			vis[x][y - 1] = 1;
			PixelPoint t(x, y - 1, *(config.img.getPixel(x, y - 1)));
			os.Add(t);
		}
		if (x + 1 < W && base.distanceTo(*(config.img.getPixel(x + 1, y))) <= config.tolerance && !vis[x + 1][y]) {
			vis[x + 1][y] = 1;
			PixelPoint t(x + 1, y, *(config.img.getPixel(x + 1, y)));
			os.Add(t);
		}
		if (y + 1 < H && base.distanceTo(*(config.img.getPixel(x, y + 1))) <= config.tolerance && !vis[x][y + 1]) {
			vis[x][y + 1] = 1;
			PixelPoint t(x, y + 1, *(config.img.getPixel(x, y + 1)));
			os.Add(t);
		}
		if (x - 1 >= 0 && base.distanceTo(*(config.img.getPixel(x - 1, y))) <= config.tolerance && !vis[x - 1][y]) {
			vis[x - 1][y] = 1;
			PixelPoint t(x - 1, y, *(config.img.getPixel(x - 1, y)));
			os.Add(t);
		}
		*px = config.picker->operator()(p);
